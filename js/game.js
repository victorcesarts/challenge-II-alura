...
function verifyWrongChar(letter){
    ...
    if(lettersInSpace.length == 0){
        ...
        printWrongChar(letter)
    }else{
        for (var i = 0; i < lettersInSpace.length; i++) {
            if(lettersInSpace[i].textContent == letter){
                thereIsWrongChar = true
                break;
            }else{
                printWrongChar(letter)
